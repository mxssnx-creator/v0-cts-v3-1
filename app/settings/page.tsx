...
export default function SettingsPage() {
  ...
  const saveAllSettings = async () => {
    ...
    try {
      ...
      if (databaseSizesChanged) {
        ...
        if (!reorganizeResponse.ok) {
          console.warn("[v0] Database reorganization failed, but settings saved")
          toast.warning("Settings saved, but database reorganization failed", {
            description: "Your settings are saved, but database limits may not be updated.",
          })
        } else {
          console.log("[v0] Database reorganized successfully")
          toast.success("Database reorganized!", {
            description: "New database size limits applied.",
          })
        }
      }

      // Step 6: If engine intervals changed, restart engine
      if (engineIntervalsChanged || databaseTypeChanged) {
        console.log("[v0] Engine restart required...")
        toast.info("Restarting trade engine...", {
          description: "Applying new engine intervals.",
        })

        const resumeResponse = await fetch("/api/trade-engine/resume", { method: "POST" })
        if (!resumeResponse.ok) {
          console.warn("[v0] Failed to restart trade engine")
          toast.warning("Trade engine restart failed", {
            description: "You may need to manually restart the engine.",
          })
        } else {
          console.log("[v0] Trade engine restarted successfully")
          toast.success("Trade engine restarted!", {
            description: "Engine is now running with new intervals.",
          })
        }
      }
  ...
  return <div className="p-4">{/* Settings UI components would go here */}</div>
}
