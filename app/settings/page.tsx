...
export default function SettingsPage() {
  ...
  // Reload connections when the 'exchange' tab is active
  useEffect(() => {
    ...
    try {
      ...
      if (databaseSizesChanged) {
        ...
        if (!reorganizeResponse.ok) {
          console.warn("[v0] Database reorganization failed")
          toast.warning("Database reorganization incomplete", {
            description: "Settings saved but database reorganization encountered issues.",
          })
        } else {
      ...
      // Step 6: If engine was paused, resume it
      if (databaseSizesChanged || engineIntervalsChanged || databaseTypeChanged) {
        console.log("[v0] Resuming trade engine...")
        toast.info("Resuming trade engine...", {
          description: "Restarting with new configuration.",
        })
        ...
        if (!resumeResponse.ok) {
          console.warn("[v0] Failed to resume trade engine")
          toast.warning("Engine restart required", {
            description: "Please manually restart the trade engine from the dashboard.",
          })
        } else {
          console.log("[v0] Trade engine resumed successfully")
          toast.success("Trade engine restarted!", {
            description: "Engine is now running with new configuration.",
          })
        }
      ...
      console.log("[v0] Settings save operation completed successfully")
    } catch (error) {
      console.error("[v0] Settings save error:", error)
      setReorganizing(false)
      toast.error("Error saving settings", {
        description: error instanceof Error ? error.message : "Failed to save settings to database.",
      })
    } finally {
      setSaving(false)
    }
  ...
  const importSettings = async () => {
    ...
    input.onchange = async (e: any) => {
      ...
      try {
        ...
        const data = JSON.parse(text) // Corrected from JSON.Parse to JSON.parse
  ...
  const loadConnections = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Set the initially selected connection from local storage if available and valid
        const storedConnectionId = localStorage.getItem("activeExchangeConnection")
        if (storedConnectionId && data.connections.some((c: ExchangeConnection) => c.id === storedConnectionId)) {
          setSelectedExchangeConnection(storedConnectionId)
        } else if (data.connections.length > 0) {
          // If stored connection is not found or invalid, default to the first active/enabled connection or the first connection
          const activeConn =
            data.connections.find((c: ExchangeConnection) => c.is_active && c.is_enabled) || data.connections[0]
          if (activeConn) {
            setSelectedExchangeConnection(activeConn.id)
            localStorage.setItem("activeExchangeConnection", activeConn.id)
          }
  ...
  return (
    <AuthGuard>
      <div className="flex flex-col h-screen">
        <PageHeader title="Settings" description="Configure system-wide settings and parameters" />
        <div className="flex-1 overflow-auto p-6">
          {/* Settings content would go here */}
          <div className="text-center py-12">
            <p className="text-muted-foreground">Settings interface is being implemented</p>
          </div>
        </div>
      </div>
    </AuthGuard>
